...
|---|------|----------|-----|--------|
| D-33 | 2026-07-19 | Housekeeping: LinkedIn icon dropped where it sat beside the LinkedIn primary (GitHub-only there), Projects tags get the §4 featured-Messing variant, heroes use `min-h-dvh`, Contact meta titles de-duplicated against the "\| Leon Jäkel" template, unused shader + `three` + `lucide-react` + template SVGs removed (supersedes D-15's "kept unused" note) | Taste critique: duplicate CTA intent, spec/implementation drift, iOS viewport jump, dead weight in repo | Taste critique |
| D-32 | 2026-07-19 | `html.js` is set by a pre-paint inline script in RootShell; reveals moved from whole sections to per-item granularity with 60–100ms `transition-delay` stagger on grid items (amends D-17) | Post-hydration `classList.add` made in-view sections flicker on first load; one identical block-fade per section was the uniform reflex | Taste critique |
| D-31 | 2026-07-19 | Copy pass: em-dash density halved in EN prose, German prose dashes set as – (Halbgeviertstrich; chips/meta titles keep —), "(Kontakt)" hint removed (D-16 superseded the hint strategy), CTA label unified to "Connect on LinkedIn ↗" / "Auf LinkedIn vernetzen ↗", DE About intro grammar repaired | Taste critique: dash-heavy prose pattern-matches AI copy — a liability for exactly this positioning | Taste critique |
| D-30 | 2026-07-19 | H1s drop the "PageName —" title-tag prefix and use sentence case with a closing period; redundant page-top eyebrows removed; interior eyebrows kept and rendered as headings (`as="h2"`) where they are a section's only label | Eyebrow + H1 said the page name twice within 16px; five identical "X — Y" headlines read as SEO scaffolding, not headlines | Taste critique |
| D-29 | 2026-07-19 | Home stat row reduced to one brass stat (10+ years) + credentials list; 650+ applicants folded inline into the CCA-F line, iHack 2018 demoted to list copy (narrows D-11) | "Where the proof is" promoted an applicant count and a 2018 hackathon to display brass while CCA-F appeared twice; D-23 removed the numbers that earned the treatment | Taste critique |
| D-28 | 2026-07-19 | Nav Connect CTA demoted to the ghost/outline button; the filled Messing primary lives in page content only (the mobile menu panel may carry one — nothing else is on screen with it) | The fixed nav's fill plus any in-page primary put two filled-Messing elements in every view, breaking §1's one-per-view ration | Taste critique |
| D-27 | 2026-07-19 | Global 404 via `app/global-not-found.tsx` + `experimental.globalNotFound`: `STATUS 404 · LOST` chip, bilingual line, outline links to both homes | §4 specced the 404 status line but unmatched URLs rendered the framework's white default; two root layouts need the global convention | Taste critique |
| D-26 | 2026-07-19 | Mobile nav: below sm the header shows wordmark + MENU; the menu is a full-height Tinte Tief dot-grid panel with stacked mono links, language toggle, and the filled Connect primary. Escape closes, body scroll locks | Page links were `hidden sm:flex` with no fallback — the LinkedIn-referred phone visitor (the primary persona) had no navigation | Taste critique |
| D-25 | 2026-07-19 | Social cards: `opengraph-image.tsx` per locale renders the Ink & Brass card (wordmark lockup, hairline frame, mono subline, status chip; fonts from `assets/og/`), plus `apple-icon.tsx` ("LJ" lockup) and og/twitter metadata in both layouts | LinkedIn is the primary funnel and shares unfurled blank — the card is most visitors' first impression | Taste critique |
| D-24 | 2026-07-19 | Hero video re-encoded 15.3MB → 0.7MB (960px, H.264 CRF 30, audio track stripped, faststart) with a poster-frame first paint; playback is JS-gated, `prefers-reduced-motion` gets the still, and a mono pause chip satisfies WCAG 2.2.2 | 24.7Mbps footage autoplayed under a 70% scrim on the landing page; looping motion had no pause control and ignored reduced-motion | Taste critique |
